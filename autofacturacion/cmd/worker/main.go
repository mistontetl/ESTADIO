...
func main() {
	...
	// ─────────────────────────────────────────────
	// DB LOCAL (solo lectura si WORKER_MODE=READ_ONLY)
	// ─────────────────────────────────────────────
	...
	//  Migraciones solo si NO estás en READ_ONLY
	if mode := getenv("WORKER_MODE", "WRITE"); mode != "READ_ONLY" {
		migrations.RunBillingMigrations(db)
	} else {
		log.Println(" READ_ONLY: Migraciones deshabilitadas")
	}
