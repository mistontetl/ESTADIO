...
func (e *EstadioTicket) GetValidTicketForBilligS(ticketID string) (models.Ticket, error) {
	...
	log.Printf("ordr_id: %d", so.OrdrID)
	log.Printf("source_sale_uuid: %s", so.SourceSaleUUID)
	log.Printf("source_sale_id: %s", so.SourceSaleID)
	...
	fmt.Println(ticket.SystemID, "  ::: ", ticket.IdTicket, "  ::: ", ticket.TotalAmount, "  ::: ", ticket.DateCreatedTicket, "  ::: ")
	return ticket, nil
}
